...
class MessageServiceTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mimo_service: MimoService = MimoService(self.host, self.token, self.requests_mock)
    ...
    def test_filter_messages_by_phone_number(self):
        phone_number = "933843893"
        self.requests_mock.get.return_value = {"status_code": 200}
        result = self.message_service.list(phone_number=phone_number)
        self.assertEqual(result["status_code"], 200)
    
    def test_filter_messages_by_start_date_and_end_date(self):
        start_date = "2022-07-01"
        end_date = "2023-07-01"
        self.requests_mock.get.return_value = {"status_code": 200}
        result = self.message_service.list(start_date=start_date, end_date=end_date)
        self.assertEqual(result["status_code"], 200)
    ...
    def test_tool_make_url(self):
        ...
        self.assertEqual(url, f"{self.host}/mimosms/v1/{endpoint}?token={self.token}")
